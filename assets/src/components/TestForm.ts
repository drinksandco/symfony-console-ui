...
export class TestForm extends LitElement {
  ...
  private hydrateArguments() {
    ...
    this.inputArgumentNodes.forEach((value: Node) => {
      const inputElement: TextField = value as TextField;
  ...
  private hydrateOptionChecks() {
    ...
    this.inputCheckNodes.forEach((value: Node) => {
      const inputElement: Checkbox = value as Checkbox;
      const index = this.inputOptionValues.indexOf(inputElement.name);
      if (inputElement.checked && index === -1) {
  ...
  private async submitForm() {
    ...
    await api(`${process.env.CONSOLE_API_URL}/cli/console-ui/schedule`, {
      method: 'POST',
      headers: {
  ...
  private renderArguments() {
    return html`
      <div class="textfield-argument">
        ${this.testType.arguments.map(
          (argument: InputArgument) => html`
            <mwc-textfield
              outlined
              label="${argument.name}"
  ...
  private renderInputOptions() {
    return html`
      <div class="textfield-option">
        ${this.testType.options.map((option: InputOption) =>
          this.renderOptionField(option)
        )}
      </div>
    `;
  }
  ...
  private printCommand() {
    return `bin/console ${this.testType.name} ${this.inputArgumentValues
      .map(value => `${value} `)
      .join()
      .replaceAll(',', '')}${this.inputOptionValues
      .map(value => `${value} `)
      .join()
      .replaceAll(',', '')}`;
  }
  ...
  render() {
    return html`
      ...
      <mwc-dialog id="dialog1" class="" heading="${this.testType.name}">
        <div>
          <cli-output content="${this.printCommand()}"></cli-output>
        ...
        <mwc-button slot="secondaryAction" dialogAction="cancel" icon="close"
          >Close</mwc-button
        >
      </mwc-dialog>
    `;
